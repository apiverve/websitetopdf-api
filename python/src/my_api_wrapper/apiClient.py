...
class WebsitetopdfAPIClientError(Exception):
...
class ValidationError(WebsitetopdfAPIClientError):
...
class WebsitetopdfAPIClient:
    ...
    VALIDATION_RULES = {"url": {"type": "string", "required": True, "format": "url"}, "marginTop": {"type": "number", "required": False, "default": "0.4"}, "marginRight": {"type": "number", "required": False, "default": "0.4"}, "marginBottom": {"type": "number", "required": False, "default": "0.4"}, "marginLeft": {"type": "number", "required": False, "default": "0.4"}, "landscape": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the WebsitetopdfAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises WebsitetopdfAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise WebsitetopdfAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[WebsitetopdfAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        :raises WebsitetopdfAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise WebsitetopdfAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise WebsitetopdfAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise WebsitetopdfAPIClientError(f"Request failed: {str(e)}")
